...
// --- Hệ thống Key: đổi streak hiện tại lấy Key, streak sẽ bị tiêu về 0 sau khi đổi ---
...
export default function Manifestation({
  ...
  // Đổi streak hiện tại lấy 1 Key - streak sẽ bị tiêu về 0 sau khi đổi thành công
  const handleRedeemKey = async (tier: KeyTier) => {
    if (!currentUser) {
      showToast("Vui lòng đăng nhập để đổi Key! ✨", "info");
      return;
    }

    const threshold = KEY_TIER_META[tier].threshold;
    if (displayStreak < threshold) {
      showToast(`Cần giữ chuỗi ít nhất ${threshold} ngày để đổi Key ${KEY_TIER_META[tier].label}!`, "error");
      return;
    }
    ...
    try {
      ...
      onUpdateUser(updatedUser);
      showToast(
        `Đã đổi thành công 1 Key ${KEY_TIER_META[tier].emoji} ${KEY_TIER_META[tier].label}! Chuỗi được reset về 0, cùng bắt đầu hành trình mới nhé! 🔑`,
        "success"
      );
    } catch (err) {
      console.error(err);
      showToast("Đổi Key thất bại, thử lại nhé!", "error");
    } finally {
        ...
        {currentUser && (
          <div className="mb-6 bg-white/50 rounded-2xl p-5 border border-indigo-100/60 shadow-inner">
            ...
            <p className="text-[10px] text-slate-500 mt-1 mb-3">
              Đủ mốc là đổi được Key tương ứng — lưu ý: đổi Key sẽ tiêu hết chuỗi hiện tại về 0.
            </p>
